...
class BasicMonster:
    ...
    def take_turn(self):
        ...
        monster = self.owner
        if is_in_fov(monster.x, monster.y):
...
def render_all():
    ...
    global fov_recompute, player, objects
    global game_msgs
...
def handle_keys():
    global game_state, objects, player
...
def target_tile(max_range=None):
    ...
    while True:
        ...
        if (mouse.lbutton_pressed and is_in_fov(x, y) and
...
def closest_monster(max_range):
    ...
    for object in objects:
        if object.fighter and not object == player and is_in_fov(object.x, object.y):
...
def is_in_fov(x, y):
    return fov_map.get((x, y), 0) > 0

def compute_fov():
    global fov_map, player, caster
    fov_map.clear()
    fov_map[player.x, player.y] = 1
    caster.calculate_light(player.x, player.y, TORCH_RADIUS)

def initialize_fov():
    global fov_recompute, fov_map, caster
    ...
    def blocks_light(x, y):
        global map
        return map[x][y].block_sight

    def set_light(x, y, intensity):
        global fov_map
        fov_map[x, y] = intensity
...
def play_game():
    global objects, game_state, camera_x, camera_y
